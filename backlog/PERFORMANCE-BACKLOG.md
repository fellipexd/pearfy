...
- [ ] `PPERF-MEM-002` Soak e auditoria de ARC/cache/task retention. **Depende de:** `PPERF-HTTP-003`. **Progresso:** RSS sampling em carga atingiu 26,384 KB e estabilizou nos últimos segundos do workload de 12.5 s; isto não substitui soak nem auditoria de retenção. **Aceite:** perfil de crescimento explicado.
...
- [ ] `PPERF-QA-004` Benchmarks de longo prazo e budget por starter. **Depende de:** benchmarks de cada módulo. **Progresso:** microbenchmarks locais de cache, messaging, jobs, outbound HTTP stub e metrics em `Benchmarks/Baselines/MODULES-2026-09-25-macos-arm64.*`; HTTP também tem stress comparativo de 480 mil requisições em `HTTP-STRESS-2026-09-25-macos-arm64.*`. **Falta:** Redis/DB/broker reais, soak, CI budget e changelog por starter. **Aceite:** changelog com custo operacional.
