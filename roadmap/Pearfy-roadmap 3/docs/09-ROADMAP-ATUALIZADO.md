...
## Complemento v1.3 — Pearfy Connect

Leia `docs/connect/00-LEIA-ME-ROADMAP.md` para plano PCON-000...018. O Connect é incremental ao plano acima e não altera a opção A de banco direto multi-instância ou os requisitos do PaymentEngine. Adiciona `@RouteGroup`, compiler de contratos, SDKs iOS/Android/TypeScript, Postman separado por grupo e cURL por rota HTTP, WebSocket/gRPC tipados, TLS e payload sealed opcional, Guardian e matriz de compatibilidade de clientes publicados. O implementation status é desconhecido até inventariar a branch real.
