...
| G3 — migrations | Parcial | `PostgresSchemaCompiler` gera CREATE/add-nullable e mudanças explícitas revisáveis, bloqueia coluna obrigatória sem default/backfill e exige aprovação para drops/rename; DDL inicial/add-column passou contra PostgreSQL real. `SQLMigrationRunner` registra SHA-256 de up/down/parameters, detecta drift para IDs declarados, adota journal legado, serializa por advisory transaction lock e expõe `plan` com estados pending/applied/legacy/drift sem executar DDL de domínio. Dois clientes simultâneos, rollback, falha e drift passaram em PostgreSQL real. Faltam catálogo/artifacts completos, detecção de versões removidas/gaps, plano de rollback e CLI plan/deploy. |
...
| G11 — release | Parcial | 98 testes locais passam em Debug/Release com PostgreSQL/Redis, incluindo Social/PostgreSQL, transaction manager e plan/locking/drift de migrations; workflow macOS/Linux está configurado, mas sem execução remota e sem revisão/certificação completa de release. |
...
- `bash scripts/test-integrations.sh`: 98 testes passaram em Debug com PostgreSQL e Redis locais; inclui groups/Connect snapshot, Entity/Schema registry, UUIDv7, schema plans, migration plans/checksums/locks, Transaction Manager, Module Manager, Social graph, HTTP, DI e adapters.
- `bash scripts/test-integrations.sh -c release`: os mesmos 98 passaram em Release.
...
1. Fechar G3 com catálogo/artifacts completos, checagem de gaps/removals, rollback plans e comandos seguros de plan/deploy.
