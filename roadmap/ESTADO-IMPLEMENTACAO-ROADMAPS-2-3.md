...
| G3 — migrations | Parcial | `PostgresSchemaCompiler` gera CREATE/add-nullable e mudanças explícitas revisáveis, bloqueia coluna obrigatória sem default/backfill e exige aprovação para drops/rename; DDL inicial/add-column passou contra PostgreSQL real. `SQLMigrationRunner` agora registra SHA-256 de up/down/parameters, detecta drift para IDs declarados, adota journal legado e usa advisory transaction locks no PostgreSQL; dois clientes simultâneos, rollback e drift passaram em integração real. Faltam catálogo/artifacts completos, detecção de versões removidas/gaps, plano de rollback e CLI plan/deploy. |
...
| G11 — release | Parcial | 97 testes locais passam em Debug/Release com PostgreSQL/Redis, incluindo Social/PostgreSQL, transaction manager e locking/drift de migrations; workflow macOS/Linux está configurado, mas sem execução remota e sem revisão/certificação completa de release. |
...
- `bash scripts/test-integrations.sh`: 97 testes passaram em Debug com PostgreSQL e Redis locais; inclui groups/Connect snapshot, Entity/Schema registry, UUIDv7, schema plans, migration checksums/locks, Transaction Manager, Module Manager, Social graph, HTTP, DI e adapters.
- `bash scripts/test-integrations.sh -c release`: os mesmos 97 passaram em Release.
