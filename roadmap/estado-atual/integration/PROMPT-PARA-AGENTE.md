...
`PPERF-QA-001` foi executada neste checkout: baseline release e CSVs brutos estão em `Benchmarks/Baselines/`. A revisão final do issue requer associar os dados a um commit, pois este workspace ainda não tem Git inicializado. `PPERF-DI-001`–`003` também foram implementadas; os próximos itens só avançam quando suas dependências do roadmap principal (request scope, HTTP, Data e operação) existirem.
