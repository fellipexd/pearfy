...
**Estado do checkout:** o núcleo DI é async/actor-based; este package já contém Web/NIO, validação, autenticação, SQL/PostgreSQL, cache, messaging/jobs locais, outbound HTTP, AI chat e health/metrics. Isso não significa que as integrações externas, os gates 1.0 ou todo o roadmap original estejam concluídos. Os arquivos `docs/01-ROADMAP.md` e `docs/06-BACKLOG.md`, apontados como fonte de verdade, não estão presentes neste checkout; são necessários para confirmar a lista e ordem completa de features do roadmap original.
...
O baseline e a comparação de lookup deste checkout estão em `../Benchmarks/Baselines/`; a reprodução usa `../scripts/benchmark-di.sh`.
