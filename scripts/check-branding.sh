...
if grep -RinE --exclude='*.md' --exclude='*.zip' --exclude-dir='.build' --exclude-dir='.git' "$pattern" Package.swift Sources Tests Plugins Examples scripts .gitignore 2>/dev/null; then
